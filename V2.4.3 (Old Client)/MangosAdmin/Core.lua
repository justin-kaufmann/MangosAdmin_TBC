...
function MangosAdmin.BuildItemsetIndexAuto()
	local function CleanName(name)
		...
		return (name:gsub("%s*[dD][eE][dD][eE]$", "")
					:gsub("%s*[eE][nN][uU][sS]$", "")
					:gsub("%s*[fF][rR][fF][rR]$", "")
					:gsub("%s*[eE][sS][eE][sS]$", "")
					:gsub("%s*[iI][tT][iI][tT]$", ""))
...
if not MangosAdminDB.items or not MangosAdminDB.itemsets then return end

    local nameToItemId = {}
    for id, name in pairs(MangosAdminDB.items) do
        local clean = CleanName(name)
        if clean then
            nameToItemId[strlower(clean)] = id
        end
    end
    ...
    for id, name in pairs(MangosAdminDB.itemsets) do
        local clean = CleanName(name)
        if clean then
            setNameToId[strlower(clean)] = id
        end
    end

    if not next(nameToItemId) or not next(setNameToId) then
        DEFAULT_CHAT_FRAME:AddMessage("|cffff5555[MA]|r Keine Items oder Itemsets zum Indexieren gefunden.")
        return
    end

    MangosAdminDB.itemsetItems = {}

    local tooltip = CreateFrame("GameTooltip", "MangosAdminScanTooltip", UIParent, "GameTooltipTemplate")
    tooltip:SetOwner(UIParent, "ANCHOR_NONE")

    local setList = {}
    for setName, setId in pairs(setNameToId) do
        table.insert(setList, { name = setName, id = setId })
    end

    local index = 1
    local batchSize = 3  -- Sets pro Frame (anpassen je nach Performance)

    local f = CreateFrame("Frame")
    f:SetScript("OnUpdate", function(self, elapsed)
        local processed = 0
        while index <= #setList and processed < batchSize do
            local entry = setList[index]
            local setName, setId = entry.name, entry.id
            local items = {}

            for itemName, itemId in pairs(nameToItemId) do
                tooltip:ClearLines()
                tooltip:SetHyperlink("item:"..itemId)
                local line1 = _G["MangosAdminScanTooltipTextLeft1"]:GetText()
                if line1 and strlower(line1):find(setName, 1, true) then
                    table.insert(items, itemId)
                end
            end

            MangosAdminDB.itemsetItems[setId] = items
            DEFAULT_CHAT_FRAME:AddMessage("|cff55ff55[MA]|r Set '"..setName.."' -> "..#items.." Items erkannt")

            index = index + 1
            processed = processed + 1
        end

        if index > #setList then
            DEFAULT_CHAT_FRAME:AddMessage("|cff55ff55[MA]|r Itemset-Indexierung abgeschlossen.")
